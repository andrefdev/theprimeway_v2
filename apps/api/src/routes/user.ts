...
// ---------------------------------------------------------------------------
// GET /currency-settings — Fetch currency settings
// ---------------------------------------------------------------------------
...
const getCurrencySettingsRoute = createRoute({
  method: 'get',
  path: '/currency-settings',
  tags: ['User'],
  summary: 'Get user currency settings',
  security: [{ Bearer: [] }],
  responses: {
    200: { content: { 'application/json': { schema: currencySettingsSchema } }, description: 'Currency settings' },
    404: { content: { 'application/json': { schema: errorResponse } }, description: 'User not found' },
    500: { content: { 'application/json': { schema: errorResponse } }, description: 'Internal error' },
  },
})

userRoutes.openapi(getCurrencySettingsRoute, (async (c: any) => {
  const { userId } = c.get('user')

  try {
    const data = await userService.getCurrencySettings(userId)
    if (!data) return c.json({ error: 'User not found in database' }, 404)
    return c.json({ data }, 200)
  } catch (error) {
    console.error('[CURRENCY_SETTINGS_GET]', error)
    return c.json({ error: 'Internal Error' }, 500)
  }
}) as any)

// ---------------------------------------------------------------------------
// POST /currency-settings — Create/update currency settings
// ---------------------------------------------------------------------------
...
const postCurrencySettingsRoute = createRoute({
  method: 'post',
  path: '/currency-settings',
  tags: ['User'],
  summary: 'Create or update currency settings',
  security: [{ Bearer: [] }],
  request: {
    body: { content: { 'application/json': { schema: updateCurrencyBody } } },
  },
  responses: {
    200: { content: { 'application/json': { schema: currencySettingsSchema } }, description: 'Updated currency settings' },
    500: { content: { 'application/json': { schema: errorResponse } }, description: 'Internal error' },
  },
})

userRoutes.openapi(postCurrencySettingsRoute, (async (c: any) => {
  const { userId } = c.get('user')

  try {
    const body = c.req.valid('json')
    const data = await userService.updateCurrencySettings(userId, body)
    return c.json({ data }, 200)
  } catch (error) {
    console.error('[CURRENCY_SETTINGS_POST]', error)
    return c.json({ error: 'Internal Error' }, 500)
  }
}) as any)

// ---------------------------------------------------------------------------
// DELETE /currency-settings — Reset currency settings
// ---------------------------------------------------------------------------
const deleteCurrencySettingsRoute = createRoute({
  method: 'delete',
  path: '/currency-settings',
  tags: ['User'],
  summary: 'Delete / reset currency settings',
  security: [{ Bearer: [] }],
  responses: {
    200: { content: { 'application/json': { schema: messageResponse } }, description: 'Settings reset' },
    500: { content: { 'application/json': { schema: errorResponse } }, description: 'Internal error' },
  },
})

userRoutes.openapi(deleteCurrencySettingsRoute, async (c) => {
  const { userId } = c.get('user')

  try {
    await userService.deleteCurrencySettings(userId)
    return c.json({ message: 'Settings reset successfully' }, 200)
  } catch (error) {
    console.error('[CURRENCY_SETTINGS_DELETE]', error)
    return c.json({ error: 'Internal Error' }, 500)
  }
})
