...
import { useBadgeOverflow } from '@/shared/hooks/use-badge-overflow'
import { useDebouncedCallback } from '@/shared/hooks/use-debounced-callback'
import { getCellKey, getLineCount } from './utils'
import { cn } from '@/shared/lib/utils'
import type { CellVariantProps, FileCellData } from './types'
...
export function ShortTextCell<TData>({
  ...
  const cellOpts = cell.column.columnDef.meta?.cell as
    | { linkBase?: string; onOpenRow?: (row: unknown) => void }
    | undefined
  const linkBase = cellOpts?.linkBase
  const baseSegment = React.useMemo(() => {
  ...
  const onOpenRow = React.useCallback(
    (event: React.MouseEvent) => {
      ...
      if (cellOpts?.onOpenRow) {
        cellOpts.onOpenRow(cell.row.original)
        return
      }
      if (!rowId) return
      const base = linkBase ?? baseSegment ?? '/'
      const target = base.endsWith('/') ? `${base}${rowId}` : `${base}/${rowId}`
      navigate({ to: target })
    },
    [navigate, rowId, linkBase, baseSegment, cellOpts, cell.row.original],
  )
  ...
  return (
    ...
    >
      ...
      {columnId === 'title' && !isEditing && rowId ? (
        <button
          aria-label={t('actions.open')}
          tabIndex={-1}
          onClick={onOpenRow}
          className="hover:bg-muted text-muted-foreground absolute top-1/2 right-1 grid size-6 -translate-y-1/2 place-items-center rounded opacity-0 group-hover:opacity-100"
        >
...
export function MultiSelectCell<TData>({
    ...
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === 'Enter') {
        ...
        const exists =
          ...
          options.some((opt) => opt.value === label || opt.label === label)
        if (!exists) {
  ...
  }, [readOnly, searchValue, selectedValues, options])
              ...
              <CommandList className="max-h-full">
                ...
                {canCreate ? (
                  <CommandGroup>
                    ...
                    >
                      {t('createTag', { label: searchValue.trim() })}
                    </CommandItem>
                  </CommandGroup>
                ) : null}
                {selectedValues.length > 0 && (
...
export function DateCell<TData>({
  ...
  const selectedDate = value
    ? new Date(
        (value.includes('T') ? value.split('T')[0] : value) + 'T00:00:00',
      )
    : undefined
