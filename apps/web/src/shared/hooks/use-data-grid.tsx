...
import { DataGridCell } from '@/shared/components/data-grid/DataGridCell'
import {
  getCellKey,
  getRowHeightValue,
  parseCellKey,
} from '@/shared/components/data-grid/utils'
import type {
  CellPosition,
  ContextMenuState,
  NavigationDirection,
...
export type DataGridMeta<TData> = TableMeta<TData> & {
  addRowLabel?: string
...
interface UseDataGridProps<TData> extends Omit<
  TableOptions<TData>,
  'pageCount' | 'getCoreRowModel' | 'meta'
> {
...
function useDataGrid<TData>({
  ...
  const onRowAdd = React.useCallback(
    async (event?: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (result) {
        requestAnimationFrame(() => {
          ...
          const targetIndex = Math.min(
            result.rowIndex ?? rows.length,
            Math.max(rows.length - 1, 0),
          )
          onScrollToRow({ rowIndex: targetIndex, columnId: result.columnId })
        })
        return
      }
      ...
      requestAnimationFrame(() => {
        const rows = tableRef.current?.getRowModel().rows ?? []
        onScrollToRow({ rowIndex: Math.max(rows.length - 1, 0) })
      })
    },
