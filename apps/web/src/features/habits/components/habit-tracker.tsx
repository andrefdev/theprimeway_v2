...
export function HabitTracker({ habits, onToggle, onEdit }: HabitTrackerProps) {
  ...
  const dayCount = isMobile ? DAYS_MOBILE : DAYS_DESKTOP

  const dates = useMemo(() => {
    const today = new Date()
    return Array.from({ length: dayCount }, (_, i) => {
      const d = subDays(today, dayCount - 1 - i)
      return {
        date: d,
        key: format(d, 'yyyy-MM-dd'),
        label: format(d, 'd'),
        dayName: format(d, 'EEE').charAt(0),
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              <th className="sticky left-0 z-10 bg-muted/30 px-3 py-2 text-left font-medium text-muted-foreground min-w-[140px]">
                {t('habit')}
              </th>
              {dates.map((d) => (
                <th
                  key={d.key}
                  className={`px-1 py-2 text-center font-normal min-w-[32px] ${
                    d.isToday ? 'bg-primary/10' : ''
                  }`}
                >
                  <div className="text-[10px] text-muted-foreground">{d.dayName}</div>
                  <div className={`text-[11px] ${d.isToday ? 'text-primary font-bold' : 'text-foreground'}`}>
                    {d.label}
                  </div>
                </th>
              ))}
              <th className="px-3 py-2 text-center font-medium text-muted-foreground min-w-[50px]">
                {t('streak')}
              </th>
            </tr>
          </thead>
  ...
  return (
    <tr className="border-b border-border/50 hover:bg-muted/20 transition-colors">
      ...
      {dates.map((d) => {
        ...
        return (
          <td key={d.key} className={`px-1 py-2 text-center ${d.isToday ? 'bg-primary/5' : ''}`}>
            <button
              type="button"
              disabled={disabled}
